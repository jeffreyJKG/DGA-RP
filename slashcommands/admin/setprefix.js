...
module.exports.run = async (client, inter) => {
    ...
    if(db.get(`lang_${inter.guildId}`) === 'NL'){
        ...
        const prefix = inter.options.getString('newprefix');
        db.set(`prefix_${inter.guildId}`, prefix)

        const succesEmbed = new Discord.MessageEmbed()
            .setDescription(`De prefix voor de normale commando\'s zal nu \`${prefix}\` zijn.`)
            .setColor(db.get(`color_${inter.guildId}`))
            .setAuthor({ name: 'Prefix Aangepast!', iconURL: inter.guild.iconURL({ dynamic: true }) })
        ...
        const succesEmbed = new Discord.MessageEmbed()
            .setDescription('The prefix for the normal commands will now be \`${prefix}\`.')
            .setColor(db.get(`color_${inter.guildId}`))
            .setAuthor({ name: 'Prefix Adjusted!', iconURL: inter.guild.iconURL({ dynamic: true }) })
        return await inter.reply({ embeds: [succesEmbed], ephemeral: true })
